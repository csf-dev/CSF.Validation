...
namespace CSF.Validation.Messages
{
    ...
    public class FailureMessageValidationResultPopulatorTests
    {
                                                                                         ...
                                                                                         ResolvedValidationOptions options)
        {
            ...
            Mock.Get(provider).Setup(x => x.GetRuleResultWithMessageAsync(ruleResult1, default)).Returns(Task.FromResult(ruleResult4));
            Mock.Get(provider).Setup(x => x.GetRuleResultWithMessageAsync(ruleResult2, default)).Returns(Task.FromResult(ruleResult5));
            Mock.Get(provider).Setup(x => x.GetRuleResultWithMessageAsync(ruleResult3, default)).Returns(Task.FromResult(ruleResult6));
